...
class MatchGame extends Component {
  ...
  tick = () => {
    ...
    if (time !== 0) {
      ...
      clearInterval(this.timerId)
      this.setState({isGameOnProgress: false})
    }
  }
  ...
  onChangeMatchImage = id => {
    const {isGameOnProgress, matchImage, score} = this.state
  ...
  getThumbnails = () => {
    ...
    return (
      <div className="match-game-responsive-container">
        <img src={matchImage.imageUrl} className="match-image" alt="match" />
        <ul className="match-game-tabs-container">
        ...
        <ul className="thumbnails-list-container">
          {filteredThumbnails.map(eachImage => (
            <ThumbNailsList
              eachImage={eachImage}
              key={eachImage.id}
              onChangeMatchImage={this.onChangeMatchImage}
            />
          ))}
        </ul>
      </div>
  ...
  getScorecard = () => {
    ...
    return (
      <div className="scorecard-responsive-container">
        <img
  ...
  render() {
    const {isGameOnProgress, score, time} = this.state
    return (
      <div className="match-game-bg-container">
        <NavBar score={score} time={time} getTime={this.getTime} />
        <div className="match-game-container">
          {isGameOnProgress ? this.getThumbnails() : this.getScorecard()}
        </div>
      </div>
    )
  }
